...
it('can add the star name and star symbol properly', async() => {
    let tokenId = 15;
    let instance = await StarNotary.deployed();
    await instance.createStar('Hello Star!', tokenId, {from: accounts[0]})
    assert.equal(await instance.name(), 'Korean Star');
    assert.equal(await instance.symbol(), 'STAR');
});

it('lookUptokenIdToStarInfo should return name of the star given _tokenId', async() => {
    let tokenId = 8;
    let instance = await StarNotary.deployed();
    await instance.createStar('Hello Star!', tokenId, {from: accounts[0]})
    assert.equal(await instance.lookUptokenIdToStarInfo(tokenId), 'Hello Star!');
});
...
it('lets 2 users exchange stars', async() => {
    ...
    let tokenId2 = 21;
    let instance = await StarNotary.deployed();
    await instance.createStar('Hello id1 Star!', tokenId1, {from: accounts[1]});
    await instance.createStar('Hello id2 Star!', tokenId2, {from: accounts[2]});
